    ...
    return []scriptTestCase{
		{
			...
			root: map[string]interface{}{
				"/home/user/.local/share/chezmoi/run_true.#.bat":
		...
		},
		{
			name: "simple_once",
			root: map[string]interface{}{
				"/home/user/.local/share/chezmoi/run_once_true.#.bat":
					"@echo foo>>" + filepath.Join(tempDir, "evidence") + "\n",
			},
            tests: []vfst.Test{
                vfst.TestPath(filepath.Join(tempDir, "evidence"),
		...
		},
		{
			name: "template",
			root: map[string]interface{}{
				"/home/user/.local/share/chezmoi/run_true.#.bat.tmpl":
				    "@echo {{ .Foo }}>>" + filepath.Join(tempDir, "evidence") + "\n",
			},
			data: map[string]interface{}{
                "Foo": "foo",
...
func getRunOnceFiles() map[string]interface{} {
    return map[string]interface{}{
        "/home/user/.local/share/chezmoi/run_once_foo.#.bat.tmpl":
            "@echo bar>> {{ .TempFile }}\n",
    }
}
