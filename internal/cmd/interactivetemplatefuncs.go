...
func (c *Config) promptMultichoiceInteractiveTemplateFunc(prompt string, choices any, args ...[]string) []string {
	if len(args) > 1 {
		panic(fmt.Errorf("want 2 or 3 arguments, got %d", len(args)+2))
	}
	...
	choiceStrs := mustValue(anyToStringSlice(choices))
	return mustValue(c.promptMultichoice(prompt, choiceStrs, args...))
}
