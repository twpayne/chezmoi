...
func (c *Config) runExecuteTemplateCmd(cmd *cobra.Command, args []string) error {
	...
	if c.executeTemplate.init {
		...
		promptMultichoiceInitTemplateFunc := func(prompt string, multichoices any, args ...string) []string {
		...
		promptMultichoiceOnceInitTemplateFunc := func(m map[string]any, path any, prompt string, choices []any, args ...string) []string {
			nestedMap, lastKey := mustValues(nestedMapAtPath(m, path))
			if value, ok := nestedMap[lastKey]; ok {
				if stringValue, ok := value.(string); ok {
